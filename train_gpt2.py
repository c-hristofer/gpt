...
class DataLoaderLite:
    ...
    def next_batch(self):
        ...
        if self.current_position + (B * T * self.num_processes + 1) > len(self.tokens):
            ...
            self.tokens = load_tokens(self.shards[self.current_shard])
            self.curent_position = B * T * self.process_rank
        return x, y
...
if ddp:
    # Use of DDP demanda CUDA at this point, so we set the device appropriately according to rank
